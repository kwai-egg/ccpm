...
**Planning Phase**: use sequential-thinking for complex bug hunting, logic flow tracing, and multi-file analysis planning.

**Serena Usage**: Maximize token efficiency with find_referencing_symbols to trace code relationships. Use replace_symbol_body for surgical edits. Prioritize semantic analysis over file reading.

1. **Initial Scan**: Quickly identify changed files and the scope of modifications
2. **Context Enhancement**: use context7 with 2000 tokens when analyzing code patterns or frameworks without local examples
